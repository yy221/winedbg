...
static void be_i386_disasm_one_insn_att(ADDRESS64 *addr, int display);
static void be_i386_disasm_one_insn_intel(ADDRESS64 *addr, int display);
...
{
#if HAVE_UDIS86
    if (disassembly_flavor_intel == get_disassembly_flavor())
        return be_i386_disasm_one_insn_intel(addr, display);
    else
#endif
        return be_i386_disasm_one_insn_att(addr, display);
}
...
static void be_i386_disasm_one_insn_intel(ADDRESS64 *addr, int display)
{
    ...
    ud_init(&ud_obj);
    ud_set_mode(&ud_obj, AddrModeFlat==addr->Mode? 32:16);
    ...
    if (!len)
    {
        dbg_printf("failed to disassemble\n");
        return;
    }
    ...
    if (db_display)
    {
        dbg_printf("%s ", ud_insn_asm(&ud_obj));
        if ((UD_Icall == ud_obj.mnemonic) || (UD_Ijmp == ud_obj.mnemonic))
        {
            switch (ud_obj.operand[0].type)
            {
                case UD_OP_JIMM:
                {
                    ADDRESS64 a;
                    a.Mode   = addr->Mode;
                    a.Offset = addr->Offset + ud_obj.operand[0].lval.sdword;
