...
#if HAVE_UDIS86
static db_addr_t db_disasm_intel(db_addr_t addr)
{
    int len = 0;
    ud_t ud_obj;
    ...
    dbg_read_memory(addr, buffer, sizeof(buffer));
        ...
        if ((UD_Icall == ud_obj.mnemonic) || (UD_Ijmp == ud_obj.mnemonic))
        {
            switch (ud_obj.operand[0].type)
            {
                case UD_OP_JIMM:
                {
                    ADDRESS64 a;
                    a.Mode   = AddrModeFlat;
                    a.Offset = addr + ud_obj.operand[0].lval.sdword;
...
void be_x86_64_disasm_one_insn(ADDRESS64 *addr, int display)
{
    ...
    if (disassembly_flavor_intel == get_disassembly_flavor())
        addr->Offset = db_disasm_intel(addr->Offset, TRUE);
    else
#endif
        addr->Offset = db_disasm_att(addr->Offset);
}
