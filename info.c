...
void info_win32_handle(HWND handle)
{
    ...
    if (!dbg_curr_process)
    {
        dbg_printf("Cannot get handle info while no process is loaded\n");
        return;
    }

    /* Duplicate the handle so we can query it. */
    if (NtDuplicateObject(
        dbg_curr_process->handle,
        handle,
        GetCurrentProcess(),
        &dupHandle,
        0,
    ...
    {
        dbg_printf("failed to duplicate handle!\n");
        return;
    }

    /* Query the object type. */
    objectTypeInfo = (POBJECT_TYPE_INFORMATION)malloc(0x1000);
    if (NtQueryObject( dupHandle, ObjectTypeInformation, objectTypeInfo, 0x1000, NULL))
    {
        dbg_printf("failed to allocate memory!\n");
        CloseHandle(dupHandle);
        return;
    }

    objectNameInfo = malloc(0x1000);
    if (NtQueryObject(dupHandle,
                ObjectNameInformation, objectNameInfo, 0x1000, &returnLength))
    {
        /* Reallocate the buffer and try again. */
        objectNameInfo = realloc(objectNameInfo, returnLength);
        if (NtQueryObject( dupHandle, ObjectNameInformation,
            objectNameInfo, returnLength, NULL))
        {
            /* We have the type name, so just display that. */
            dbg_printf("handle:%p, type name:%s\n", handle,
                wine_dbgstr_wn(objectTypeInfo->TypeName.Buffer, objectTypeInfo->TypeName.Length/2));
            free(objectTypeInfo);
            free(objectNameInfo);
            CloseHandle(dupHandle);
            return;
    ...
    if (objectName.Length)
    {
        ...
        dbg_printf("handle:%p, type name:%s, name:%s\n", handle,
         wine_dbgstr_wn(objectTypeInfo->TypeName.Buffer, objectTypeInfo->TypeName.Length/2),
         wine_dbgstr_wn(objectName.Buffer, objectName.Length/2));
    }
    else
    {
        dbg_printf("handle:%p, type name:%s\n", handle,
